...
function JournalVoucherFormContent() {
  ...
  const [loadingPending, setLoadingPending] = useState(isEditingPending);
  ...
  useEffect(() => {
    ...
    api
      .getPendingVoucher(pendingId)
      .then((v) => {
        if (cancelled) return;
        setDebitAccountId(v.debitAccountId != null ? String(v.debitAccountId) : '');
        setCreditAccountId(v.creditAccountId != null ? String(v.creditAccountId) : '');
        setDebitCategoryId(v.debitAccount ? String(v.debitAccount.categoryId) : '');
        setCreditCategoryId(v.creditAccount ? String(v.creditAccount.categoryId) : '');
        setAmount(String(v.amount ?? ''));
        setVoucherDate(v.date ?? todayInputValue());
        setPredictedNumber(v.number ?? null);
      ...
      .catch((err) => {
        if (!cancelled) {
          setError(err instanceof Error ? err.message : 'Failed to load pending voucher');
        }
      })
      .finally(() => {
  ...
  async function handleSave() {
    ...
    try {
      if (isEditingPending && pendingId != null) {
        await api.updatePendingVoucher(pendingId, {
        ...
        setMessage('Pending journal voucher updated.');
      ...
      setMessage('Journal voucher submitted for approval.');
      setAmount('');
      setReference('');
      setDescription('');
  ...
  return (
    <FormPageShell
      titleRef={titleRef}
      title={isEditingPending ? 'Edit Pending Journal Voucher' : 'Journal Voucher'}
      panelClassName="max-w-4xl"
    >
      {loadingPending ? <p className="mb-3 text-sm text-textMuted">Loading pending voucher…</p> : null}
      <div className="jv-form overflow-visible">
        ...
        <FormActionFooter
          error={error}
          message={message}
          primaryLabel={isEditingPending ? 'Update pending' : 'Save & Post'}
          savingLabel="Saving…"
          saving={saving || loadingPending}
          primaryType="button"
          primaryRef={saveRef}
          primaryTabIndex={9}
          onPrimaryClick={() => void handleSave()}
          onMinimize={isEditingPending ? undefined : handleMinimize}
...
function BatchVoucherFormContent({ kind }: { kind: 'payment' | 'receipt' }) {
  ...
  async function handleUpdatePending() {
    ...
    try {
      ...
      setMessage('Pending voucher updated.');
      navigate('/system/approvals');
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to update pending voucher');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleSaveAll() {
    ...
    try {
      ...
      setMessage(`Submitted ${createdVouchers.length} voucher(s) for approval.`);
      setQueuedItems([]);
      setAmount('');
      setReference('');
      setDescription('');
  ...
  const titleText = isEditingPending
    ? `Edit Pending ${VOUCHER_PAGE_TITLES[kind] ?? 'Voucher'}`
    : (VOUCHER_PAGE_TITLES[kind] ?? 'Voucher');
  ...
  return (
    ...
    >
      ...
      <div className="overflow-visible">
        <div className={`voucher-batch-form ${isEditingPending ? '' : 'voucher-batch-split'}`}>
          <div className={isEditingPending ? '' : 'voucher-batch-split-form'}>
            <InvoiceFormSection label={isEditingPending ? 'Pending voucher details' : 'New voucher details'}>
              <InvoiceHeaderRow>
                <InvoiceField>
                  <FieldLabel>Date</FieldLabel>
                  <TextInput
              ...
              <InvoiceField>
                ...
                <TextInput
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                      e.preventDefault();
                      if (isEditingPending) void handleUpdatePending();
                      else handleAddToGrid();
                    }
                  }}
                />
              </InvoiceField>
            ...
            {isEditingPending ? (
              <FormActionFooter
                message={message}
                primaryLabel="Update pending"
                savingLabel="Saving…"
                saving={saving || loadingPending}
                primaryType="button"
                primaryRef={saveRef}
                onPrimaryClick={() => void handleUpdatePending()}
                onClose={() => navigate('/system/approvals')}
              />
            ) : (
