...
function configureStableUserDataPath(): void {
  ...
  const brandedDir = path.join(appData, APP_NAME);
  const legacyDir = path.join(appData, LEGACY_APP_NAME);
  const brandedDb = path.join(brandedDir, 'data', 'sheraztrader.db');
  const legacyDb = path.join(legacyDir, 'data', 'sheraztrader.db');

  if (fs.existsSync(legacyDb) && !fs.existsSync(brandedDb)) {
